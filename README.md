...
> **Tip — business window.** Enforcement is always on and spec-driven, so a few
...
| POST | `/admin/business-window` | Update business window config (`PUT` is accepted as an alias) |
...
> endpoints (`POST /admin/reset`, `POST /admin/business-window`) and
...
| `PONTES_MOCK_BUSINESS_WINDOW_ALWAYS_OPEN` | `false` | Business-window enforcement is **always on** and spec-driven: each official operation is accessible only in the windows its spec lists (e.g. bridge payments = *Open for All* only; transfer creation = *Open for All* only), derived from the stored business day in **Frankfurt** (`Europe/Berlin`) time and returning `403 HL-BW-001` otherwise. The default day is Open-for-All for most of the day, so the common flows work at typical hours — but some operations remain time-restricted, so a call can still hit `403 HL-BW-001` (notably outside the open window). Set this to `true` to disable enforcement entirely — the single most useful switch for a first-time user or CI at odd hours. |
| `PONTES_MOCK_NETWORK_ID` | `mock-pontes` | Value echoed as `debitedNetworkID`/`creditedNetworkID` on transfer responses (`POST/GET .../rvs/transactions-requests|transactions-drafts`) — this mock only ever models a single network. |
| `PONTES_JWT_AUDIENCE_ALLOWLIST` | `esydlt-web-app-u2a,esydlt-backend-service` | Comma-separated list of JWT `aud` values accepted on `/dlt` routes (issue #118). The token endpoint accepts any `client_id` and mints `aud=[clientId, "account"]`/`azp=clientId` accordingly (mirroring real `utest` captures), but only tokens whose `aud` intersects this list pass the audience gate — everything else gets a `401 { status, message: "session is not valid", code: "unauthorized" }`, reproducing the real (undocumented) `utest` behavior of rejecting `esydlt-web-app` tokens on some endpoints despite Table U listing it as the client for several profiles. See [`docs/RECIPES.md`](docs/RECIPES.md) for details. |
